  ...
  return (
      ...
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          {navLinks.map((link)=>(
            <li key={link.id} className={`${active===link.id? "text-white text-glow":"text-secondary"} font-poppins font-medium cursor-pointer text-[16px]`} onClick={()=> {setToggle(!toggle);setActive(link.title);}}>
              <a href={`#${link.id}`}>{link.title}</a>
            </li>
          ))}
          </ul>
